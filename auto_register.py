...
class ReferralRegistration:
    ...
    def get_random_proxy(self):
        ...
        # Always require proxy - removed option to work without proxy
        if self.working_proxies:
            return random.choice(self.working_proxies)
        elif self.proxies:  # Fall back to all proxies if no working ones
            return random.choice(self.proxies)
        
        logging.error("No proxies available. Unable to proceed.")
        return None
    ...
    def register_account(self, user_info):
        ...
        try:
            ...
            response = self.session.post(form_action, data=form_data, timeout=30, verify=False)
    ...
    def run_campaign(self, num_registrations=100):
        ...
        # Always use proxies - no option to disable proxy
        self.use_proxy = True
        ...
        for i in range(num_registrations):
            ...
            # Random delay between registrations - shorter delays as requested
            delay = random.uniform(10, 12)  # Reduced delay to 10-12 seconds
